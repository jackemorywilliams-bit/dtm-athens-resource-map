...
**Brand assets** (`brand/`, mirrored in `docs/brand/`) are regenerated by
`python3 tools/build_brand_assets.py` (needs Pillow + numpy; raw logo sources
configurable via env vars). Occasional-use only, deliberately outside
build.py's critical path. Notes: the Emerging Leaders mark is a typographic
placeholder (no official asset exists), the DTA mark is typographic, and the
ODB mark comes from a low-resolution raster; request vector originals from DTM
administration before any large-format use.
